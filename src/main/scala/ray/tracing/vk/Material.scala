...
case class LambertMaterial(albedo: Vec3) extends Material {
  override def scatter(
    ...
    val target: Vec3 =
      hitRecord.p + hitRecord.normal + Main.randomInUnitSphare()
...
case class MetalMaterial(albedo: Vec3, f: Float) extends Material {
  ...
  override def scatter(
    ...
    val reflected: Vec3 =
      MaterialUtils.reflect(rIn.direction.unitVector, hitRecord.normal)
    val scattered: Ray =
      Ray(hitRecord.p, reflected + Main.randomInUnitSphare() * fuzz)
